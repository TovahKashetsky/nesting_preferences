  ...
  mutate(
    ...
    mutate(
      Variable = dplyr::recode(Variable,
                               '(Intercept)' = 'Intercept',
                               'scale(Osmia_ps)' = 'Neighbouring congeneric nests, including pseudo nests',
                               'scale(Osmia_no_ps)' = 'Neighbouring congeneric nests, excluding pseudo nests',
                               'scale(Meg_ps)' = 'Neighbouring congeneric nests, including pseudo nests',
                               'scale(Meg_no_ps)' = 'Neighbouring congeneric nests excluding pseudo nests',
                               'scale(Osmia_active_mothers)' = 'Neighbouring active host mothers',
                               'scale(Meg_active_mothers)' = 'Neighbouring active host mothers',
                               ...
                               'scale(DP_os_prior):scale(NDP_ps_os_prior)' = 'Neighbouring congeneric nests with detectable parasites * Neighbouring congeneric nests with no detectable parasites',
                               ...
                               'scale(DP_meg_prior):scale(NDP_ps_meg_prior)' = 'Neighbouring congeneric nests with detectable parasites * Neighbouring congeneric nests with no detectable parasites',
...
format_table_function_par <- function(df) {
  df %>%
    mutate(term_2 = ifelse(term == 'sd__(Intercept)', group, term)) %>%
    select(-term, -group) %>%
    rename(
  ...
  mutate(
    ...
    mutate(
      Variable = dplyr::recode(Variable,
                               '(Intercept)' = 'Intercept',
                               'scale(Osmia_ps)' = 'Neighbouring host nests, including pseudo nests',
                               'scale(Osmia_no_ps)' = 'Neighbouring host nests, excluding pseudo nests',
                               'scale(Meg_ps)' = 'Neighbouring host nests, including pseudo nests',
                               'scale(Meg_no_ps)' = 'Neighbouring host nests, excluding pseudo nests',
                               'scale(Osmia_active_mothers)' = 'Neighbouring active host mothers',
                               'scale(Meg_active_mothers)' = 'Neighbouring active host mothers',
                               'Cell_parasitized' = 'Cell parasitism',
                               'Nest_parasitizedparasitized' = 'Nest parasitism (parasitized)',
...
mod_2i_par_agg_os_ps_table <- tidy_function_glmer(mod_2i_par_agg_os_ps) %>% format_table_function_par() 
mod_2ii_par_agg_os_no_ps_table <- tidy_function_glmer(mod_2ii_par_agg_os_no_ps) %>% format_table_function_par() 
mod_2iii_par_agg_os_active_table <- tidy_function_glmer(mod_2iii_par_agg_os_active) %>% format_table_function_par() 
mod_2iv_par_agg_meg_ps_table <- tidy_function_glmer(mod_2iv_par_agg_meg_ps) %>% format_table_function_par()
mod_2v_par_agg_meg_no_ps_table <- tidy_function_glmer(mod_2v_par_agg_meg_no_ps) %>% format_table_function_par() 
mod_2vi_par_agg_meg_active_table <- tidy_function_glmer(mod_2vi_par_agg_meg_active) %>% format_table_function_par() 
...
**Table S5.** Summary of the GLMM for prediction 1, model 1_i testing for an attraction to congenerics in nesting *Osmia*, including pseudo nests as congenerics, n = 161 nests.
...
table_1i <- tribble(
  ~Effect,   ~Variable,
  "fixed",   "Neighbouring congeneric nests including pseudo nests",
...
**Figure S2.** Coefficients from the *Osmia* GLMMs using simulated data (grey bars) and the coefficient from the GLMM using observed data (red line) for the number of neighbouring congeneric nests (including pseudo nests) from model 1_i.

**Table S6.** Summary of the GLMM for prediction 1, model 1_ii testing for an attraction to congenerics in nesting *Osmia*, excluding pseudo nests as congenerics, n = 161 nests.
...
table_1ii <- tribble(
  ~Effect,   ~Variable,
  "fixed",   "Neighbouring congeneric nests excluding pseudo nests",
  "fixed",   "Number of days with appropriate weather for bee activity",
  "fixed",   "Day of year I",
  "fixed",   "Day of year II",
  "fixed",   "Site",
...
**Figure S3.** Coefficients from the *Osmia* GLMMs using simulated data (grey bars) and the coefficient from the GLMM using observed data (red line) for the number of neighbouring congeneric nests (excluding pseudo nests) from model 1_ii.

**Table S7.** Summary of the GLMM for prediction 1, model 1_iii testing for an attraction to congenerics in nesting *Megachile*, including pseudo nests as congenerics, n = 137 nests.
...
**Figure S4.** Coefficients from the *Megachile* GLMMs using simulated data (grey bars) and the coefficient from the GLMM using observed data (red line) for the number of neighbouring congeneric nests (including pseudo nests) from model 1_iii.

**Table S8.** Summary of the GLMM for prediction 1, model 1_iv testing for an attraction to congenerics in nesting *Megachile*, excluding pseudo nests as congenerics, n = 137 nests.
...
**Figure S5.** Coefficients from the *Megachile* GLMMs using simulated data (grey bars) and the coefficient from the GLMM using observed data (red line) for the number of neighbouring congeneric nests (excluding pseudo nests) from model 1_iv.

**Table S9.** Summary of the GLMM output from model 2_i testing for an attraction to *Osmia* nests of brood parasites (*Sapyga*, *Melittobia*), including pseudo nests as *Osmia,* n = 286 cells. Asterisks indicate significance for the following tables.
...
**Table S10.** Summary of the GLMM output from model 2_ii for prediction 2, testing an attraction to *Osmia* nests of brood parasites (*Sapyga*, *Melittobia*), excluding pseudo nests as *Osmia*, n = 286 cells.
...
**Table S11.** Summary of the GLMM output from model 2_iii for prediction 2, testing an attraction to active *Osmia* nests of brood parasites (*Sapyga*, *Melittobia*), excluding pseudo nests as *Osmia,* n = 286 cells.
...
```{r Model_2_ii_figure, echo=FALSE}
...
**Table S12.** Summary of the GLMM output from model 2_iv for prediction 2, testing an attraction to *Megachile* in brood parasites (*Meloid*, *Melittobia*, *Chalcid*, and *Coelioxys*), including pseudo nests as *Megachile*, n = 142 cells.
...
**Table S13.** Summary of the GLMM output from model 2_v for prediction 2, testing an attraction to *Megachile* in brood parasites (*Meloid*, *Melittobia*, *Chalcid*, and *Coelioxys*), excluding pseudo nests as *Megachile*, n = 142 cells.
...
**Table S14.** Summary of the GLMM output from model 2_vi for prediction 2, testing an attraction to active *Megachile* nests of brood parasites (*Meloid*, *Melittobia*, *Chalcid*, and *Coelioxys*), excluding pseudo nests as *Megachile*, n = 142 cells.
...
**Table S15.** Summary of the GLMM output from model 3a_i for prediction 3a, testing an avoidance of parasites and an attraction to congenerics in nesting *Osmia*, including pseudo nests as congeneric nests with no detectable parasites, n = 161 nests.
...
table_3ai <- tribble(
  ~Effect,   ~Variable,
  "fixed",   "Neighbouring congeneric nests nests with detectable parasites",
  "fixed",   "Neighbouring congeneric nests nests with no detectable parasites, including pseudo nests",
  "fixed",   "Number of days with appropriate weather for bee activity",
  "fixed",   "Day of year I",
  "fixed",   "Day of year II",
  "fixed",   "Site",
...
**Figure S6.** Coefficients from the *Osmia* GLMMs using simulated data (grey bars) and the GLMM using observed data (red line) from model 3a_i, for A) or neighbouring congeneric nests with detectable parasites, B) or neighbouring congeneric nests with no detectable parasites (including pseudo nests), and C) the interaction between these two terms.

**Table S16.** Summary of the GLMM output from model 3a_ii for prediction 3a, testing an avoidance of parasites and an attraction to congenerics in nesting *Osmia*, excluding pseudo nests as congeneric nests with no detectable parasites, n = 161 nests.
...
table_3aii <- tribble(
  ~Effect,   ~Variable,
  "fixed",   "Neighbouring congeneric nests nests with detectable parasites",
  "fixed",   "Neighbouring congeneric nests nests with no detectable parasites, excluding pseudo nests",
  "fixed",   "Number of days with appropriate weather for bee activity",
  "fixed",   "Day of year I",
  "fixed",   "Day of year II",
  "fixed",   "Site",
...
**Figure S7.** Coefficients from the *Osmia* GLMMs using simulated data (grey bars) and the GLMM using observed data (red line) from model 3a_ii, for A) or neighbouring congeneric nests with detectable parasites, B) or neighbouring congeneric nests with no detectable parasites (excluding pseudo nests), and C) the interaction between these two terms.

**Table S17.** Summary of the GLMM output from model 3a_iii for prediction 3a, testing an avoidance of parasites and an attraction to congenerics in nesting *Megachile*, including pseudo nests as congeneric nests with no detectable parasites, n = 137 nests.
...
table_3aiii <- tribble(
  ~Effect,   ~Variable,
  "fixed",   "Neighbouring congeneric nests with detectable parasites",
  "fixed",   "Neighbouring congeneric nests nests with no detectable parasites, including pseudo nests",
  "fixed",   "Number of days with appropriate weather for bee activity",
  "fixed",   "Day of year I",
  "fixed",   "Day of year II",
...
**Figure S8.** Coefficients from the *Megachile* GLMMs using simulated data (grey bars) and the GLMM using observed data (red line) from model 3a_iii, for A) or neighbouring congeneric nests with detectable parasites, B) or neighbouring congeneric nests with no detectable parasites (including pseudo nests), and C) the interaction between these two terms.

```{r Model_3a_iii_figure, echo = FASLE}


fig <- trapnesting_meg_null %>% 
  ggplot(aes(y = New_nests_meg, x = NDP_meg_prior, color = DP_meg_prior)) +
...
**Table S18.** Summary of the GLMM output from model 3b_i for prediction 3b, testing a change in nest investment of *Osmia* in response to brood parasites, n = 74 nests.
...
**Table S19.** Summary of the GLMM output from model 3b_ii for prediction 3b, testing a change in nest investment of *Megachile* in response to brood parasites, n = 40 nests.
...
**Table S20.** Summary of the GLMM output from model 3c_i for prediction 3c, testing if brood cell parasitization predicts nest termination in *Osmia*, n = 245 cells.
...
**Table S21.** Summary of the GLMM output from model 3c_ii for prediction 3c, testing if brood cell parasitization predicts nest termination in *Megachile*, n = 136 cells.
...
**Table S22.** Summary of the GLMM output from model 3d_i for prediction 3d, testing abandoning inner walls as a response to brood parasites in *Osmia*, n = 168 nests with 28 of these nests being abandoned inner walls.
